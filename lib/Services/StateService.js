...
StateObject.prototype = {
  processRequest: function(context, cb) {
    ...
    if(functionAllowedState !== "any" && currentState !== functionAllowedState) {
      return cb(result.errorCode(402, "invalid state, must be: " + functionAllowedState));
    }
    cb(result.success());
  },
  processResult: function(context, cb) {
    ...
    if(context.$session.isValid()) {
      context.invoke_cb_result.state = this.get($session);
    } else {
      context.invoke_cb_result.state = this.start;
    }
    cb();
  },
