...
const result = require("cb-result");
const cb_result = result;

const curl = require("./curl.js");
const logger = require("../logger.js");
const config = require("../config.js");
const $emptyValue = require("./EmptyValue.js");
const jsonSchemas = require("./PrefixSchemas.js");
const SessionWrapper = require("./SessionWrapper");
const validateParams = require("./ValidateParams.js");
const additionalServices = require("./LoadServices.js");
const CircularDependency = require("./CircularDependency.js");
const defaultReservedValuesHash = require("./ReservedValues.js");
const ClassRegistryDataStructure = require("./ClassRegistry.js");

let colors = null;
try { colors = require("colors"); } catch(err) { logger.warn("module 'colors' is missing, httpVerbose will be off"); }

const allowedUntyped = require('../config.js').allowedUntyped;
const disable304Caching = require('../config.js').disable304Caching;
...
NodeTastic.prototype = {
  ...
  registerHandler: function(name, classInstance, defaultMeta) {
    ...
    if (defaultMeta) {
      classInstance.$meta = defaultMeta;
    }
  ...
  prepareReservedValues: function() {
    ...
    for(const i in defaultReservedValuesHash) {
  ...
  getRouteRequestFunction: function() {
    return (req, res) => this.routeRequest(req, res);
  },
  wrap: function(fn, name) {
    return (req, res) => this.routeSingleRequest(name, fn, req, res);
  },
  routeSingleRequest: function(name, fn, req, res) {
    var context = {
      ...
      skipInvokeFunction: function(res) {
        ...
        this.invoke_cb_result = cb_result.isType(res) ? res : cb_result.success(res);
      }
    };

    const createStep = (fn, skipCondition, directCall) => {
      return cb => {
        ...
        try {
          if(directCall) {
            ...
            fn.call(this, context, result.cb(cb, function(brk) {
              if(brk === true) { //success(true) is indication to stop the loop gracefully
                return cb(result.success(brk));
              }
              cb();
    ...
    async.series([
      createStep(this.stepHeaderOrigin),
      createStep(this.stepStartTimer),
      createStep(this.stepBeforeRequest),
      createStep(this.stepCachePreProcess),
      createStep(this.stepDefaultDisableCache),
      createStep(function(context, cb) {
        _.merge(context, {pathLeft: null, handler: fn, name: name});
        cb(result.success());
      }, null, true),
      createStep(this.stepCheckNewSession),
      createStep(this.stepGetFunctionMap),
      createStep(this.stepInvokeServicesRequest),
      createStep(this.stepFetchFunctionParameters, () => context._skipInvokeFunction),
      createStep(this.stepInvokeFunction, () => context._skipInvokeFunction),
      createStep(this.stepProcessResult),
      createStep(this.stepCachePostProcess),
      createStep(this.stepInvokeServicesResult),
      createStep(this.stepTranslateResult),
      createStep(this.stepEndTimer),
      createStep(this.stepEndRequest)
    ], result => {
      if(result && cb_result.isType(result) && !result.success) {
        ...
        if(this.setTranslateResultFn) {
          var r = this.setTranslateResultFn(result);
          if(r) {
            result = r;
          }
        }
  ...
  routeRequest: function(req, res) {
    ...
    const createStep = (fn, skipCondition) => {
      return cb => {
        if(skipCondition && skipCondition()) {
          return cb();
        }
    ...
    async.series([
      ...
      createStep(this.stepStartTimer),
      createStep(this.stepBeforeRequest),
      createStep(this.stepProcessPing),
      createStep(this.stepCachePreProcess),
      createStep(this.stepDefaultDisableCache),
      createStep(this.stepFindInstance),
      createStep(this.stepCheckNewSession),
      createStep(this.stepFillUrlParts),
      createStep(this.stepExtractUrlVariables),
      createStep(this.stepGetFunctionName),
      createStep(this.stepGetFunctionRef),
      createStep(this.stepAuthorizeFunction),
      createStep(this.stepGetFunctionMap),
      createStep(this.stepInvokeServicesRequest),
      createStep(this.stepFetchFunctionParameters, () => context._skipInvokeFunction),
      createStep(this.stepInvokeFunction, () => context._skipInvokeFunction),
  ...
  stepInvokeFunction: function(context, cb) {
    ...
    if(context.hasCb) {
      ...
      Promise.resolve(this.invoke(context.fn, context.handler, context.args)).then(function(value) {
        invokeCb(null, value);
    ...
    try {
      ...
      if(config.httpVerbose >= 2) {
        ...
        if(config.httpVerbose >= 3) {
          // print out curl
          console.log("curl: ".blue, curl(context.req));
        }
  ...
  stepGetFunctionMap: function(context, cb) {
    ...
    if(!context.meta) {
      ...
      if (!context.meta && context.handler.$meta) {
        context.meta = context.handler.$meta;
      }
    }
    cb(result.success());
  },
