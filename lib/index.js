...
NodeTastic.prototype = {
  ...
  routeRequest: function(req, res) {
    ...
    function createStep(fn) {
      return function(cb) {
        try {
          fn.call(self, context, result.legacycb(cb, function(brk) {
    ...
    async.series([
      ...
      createStep(self.stepFetchFunctionParameters),
      createStep(self.stepInvokeFunction),
      ...
      if(result && cb_result.isType(result) && !result.success) {
        // error occurred, deal with it.
        context.res.end(result);
      } else {
        if(context.resultObject !== undefined) {
          context.res.json(context.resultObject);
        } else {
          context.res.end();
        }
      }
  ...
  stepProcessResult: function(context, cb) {
    ...
    if(context.invoke_cb_result.httpCode) {
      context.res.statusCode = context.invoke_cb_result.httpCode;
      delete context.invoke_cb_result.httpCode;
    }
    if(typeof(context.resultObject) === "undefined") {
      context.resultObject = context.invoke_cb_result; // if it was translated, it was filled already
    }
    cb(result.success());
  },
    ...
    context.args.push(function(err, data) {
      ...
      if(err && cb_result.isType(err)) {
        ...
        if(err) {
          ...
          context.invoke_cb_result = cb_result.success(err);
        }
      }
      cb(result.success());
    });
  ...
  invokeServices: function(context, funcName, cb) {
    var self = this;
    async.eachSeries(additionalServices, function(service, cb) {
      if(context.handler.hasOwnProperty(additionalServices[service].constructor.AttachedPropertyName)) {
      ...
      } else if(self.globalClassInstance.hasOwnProperty(additionalServices[service].constructor.AttachedPropertyName)) {
        ...
        if(eTagObject.t) {
          ...
          if(now <= eTagObject.t || eTagObject.t === -1) {
            if(!eTagObject.c) { // time not passed and no checksum, trigger 304
              ...
              context.resultObject = undefined;
              return cb(result.success(true));
            } else {
  ...
  stepCachePostProcess: function(context, cb) {
    ...
    if(cbresult.cache && cbresult.cache.seconds) {
      ...
      var ifNoneMatch = context.req.headers["if-none-match1"] || req.headers["if-none-match"];
  ...
  stepEndRequest: function(context, cb) {
    ...
    try {
      var ms = context.ms;
      var finalResult = context.invoke_cb_result || context.resultObject;
      ...
      if(this.opts.verbose >= 2) {
        ...
        console.log(">>>>", context.req.method.toUpperCase().yellow.underline, req.url, sessionId.grey);
      }
  ...
  getHandler: function(path, cb) {
    ...
    if(!classRegistryValue.value) {
      return cb(result.errorCode(200, "couldn't find handler"));
    }
  ...
  stepGetFunctionName: function(context, cb) {
    ...
    if(context.functionName.indexOf("$") == 0) { // can not access reserved functions
      context.functionName = context.functionName.substring(1);
    }
    context.urlParts = [];
    cb(result.success());
  },
  stepGetFunctionRef: function(context, cb) {
    if(context.handler && context.handler.$getFunction) {
      ...
      var fnObj = context.handler.$getFunction({req: context.req, functionName: context.functionName}, result.cb(cb, function(data) {
        if(data) {
          context.fn = data.fn;
      ...
      context.fn = context.handler[context.functionName];
      if(!context.fn) {
  ...
  getReservedValues: function(context, param, circularDependency, cb) {
    ...
    async.timesSeries(fn.params.length - (isAsync ? 1 : 0), function(i, cb) {
      ...
      if(isAsync) {
        ...
        return cb(result.success(self.invoke(fn, null, args)));
      }
    }));
  },
