  ...
  routeSingleRequest: function(name, fn, req, res) {
    ...
    async.series([
      ...
      async.series([
        ...
        function(cb) { self.stepEndTimer(context, cb); },
        function(cb) { self.stepEndRequest(context, cb); }
      ], function() {});
    });
  },
    ...
    try {
      ...
      if(config.httpVerbose >= 2) {
        console.log('<<<<', (context.res.statusCode === 304 ? ("" + context.res.statusCode).cyan : ("" + context.res.statusCode).yellow), (cacheHit ||
                                                                                                                                           (cb_result.isType(finalResult) &&
                                                                                                                                            finalResult.success)) ? finalResult.green : finalResult.red);
      } else {
        var resultErrorText = finalResult.error;
        ...
        console.log('<<<<', (context.res.statusCode === 304 ? ("" + context.res.statusCode).cyan : ("" + context.res.statusCode).yellow) + contentLength.grey, (cacheHit ||
                                                                                                                                                                (cb_result.isType(finalResult) &&
                                                                                                                                                                 finalResult.success)) ? ("success".green) : ("error: " +
                                                                                                                                                                                                                           (cb_result.isType(finalResult) ? resultErrorText : finalResult)).red);
      }
