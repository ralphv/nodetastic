...
NodeTastic.prototype = {
  startTests: require("./AutomatedTesting"),
  ...
  registerOnEachCall: function(eachCallFn) {
    this.eachCallFn = eachCallFn;
  },
  globalServicesPlaceholder: {},
  ...
  attachGlobalService: function(service, opts) {
    return this.attachService(this.globalServicesPlaceholder, service, opts);
  },
  attachGlobalServiceInstance: function(serviceInstance) {
    return this.attachServiceInstance(this.globalServicesPlaceholder, serviceInstance);
  },
  attachService: function(classInstance, service, opts) {
    ...
    if(!classInstance.attachedServices) {
      classInstance.attachedServices = {};
    }
    if(classInstance.attachedServices[service.constructor.AttachedPropertyName]) {
      throw new Error("Service already attached");
    }
    return classInstance.attachedServices[service.constructor.AttachedPropertyName] = new service.constructor(this, opts);
  },
  attachServiceInstance: function(classInstance, serviceInstance) {
    if(!serviceInstance || !serviceInstance.IsService) {
      throw new Error("Invalid service given to AttachServiceInstance");
    }
    if(!classInstance.attachedServices) {
      classInstance.attachedServices = {};
    }
    classInstance.attachedServices[serviceInstance.constructor.AttachedPropertyName] = serviceInstance;
  },
  ...
  wrap: function(fn, name) {
    ...
    return function(req, res) {
      self.routeSingleRequest(name, fn, req, res);
    }
  },
  routeSingleRequest: function(name, fn, req, res) {
    var self = this;
    var context = {
      req: req,
      res: res,
    ...
    function createStep(fn, skipCondition, directCall) {
      return function(cb) {
        ...
        try {
          if(directCall) {
            fn(context, result.cb(cb, function(brk) {
    ...
    async.series([
      ...
      createStep(function(context, cb) {
        _.merge(context, {pathLeft: null, handler: fn, name: name});
        cb(result.success());
      }, null, true),
      createStep(self.stepCheckNewSession),
      createStep(self.stepCheckEachCallFunction),
      ...
      createStep(self.stepSendResponseHeaders)
    ], function(result) {
      if(result && cb_result.isType(result) && !result.success) {
        ...
        if(self.setTranslateResultFn) {
          var r = self.setTranslateResultFn(result, context);
          if(r) {
            result = r;
          }
        }
        context.failedResult = true;
        context.resultObject = result;
        context.res.json(result);
        self.stepEndTimer(context, function() {
          self.stepEndRequest(context, function() {});
        });
      } else {
  ...
  stepTranslateResult: function(context, cb) {
    if(this.setTranslateResultFn) {
      var res = this.setTranslateResultFn(context.invoke_cb_result, context);
  ...
  stepInvokeFunction: function(context, cb) {
    var invokeCb = function(err, data) {
    ...
    if(context.hasCb) {
      context.args.push(invokeCb);
      this.invoke(context.fn, context.handler, context.args);
    }
    else {
      q.when(this.invoke(context.fn, context.handler, context.args), function(value) {
        invokeCb(null, value);
      }, function(error) {
        invokeCb(error);
      });
  ...
  stepFetchFunctionParameters: function(context, cb) {
    ...
    async.timesSeries(map.params.length, function(i, cb) {
      if(i === map.params.length - 1) {
        // last param
        if(self.isNameCb(map.params[i].name)) {
          context.hasCb = true;
  ...
  invokeServices: function(context, funcName, cb) {
    var self = this;
    var processAttachedServices = function(context, handler, cb) {
      // first go through attached services on handler
      if(handler.attachedServices) {
        async.eachSeries(Object.keys(handler.attachedServices), function(serviceKey, cb) {
          var service = handler.attachedServices[serviceKey];
          if(service[funcName]) {
            service[funcName](context, result.cb(cb, function(brk) {
              if(brk) {
                cb(true);
              } else {
                cb();
    ...
    // process handler services then global placeholder services
    async.series([
      function(cb) {
        processAttachedServices(context, context.handler, cb);
      }, function() {
        processAttachedServices(context, self.globalServicesPlaceholder, cb);
      }], function(brk) {
    ...
    if(context.req.headers.origin) {
      ...
      if(context.req.method === "OPTIONS") {
        if(context.req.headers["access-control-request-headers"]) {
          ...
          context.res.setHeader("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, HEAD, OPTIONS"); //access-control-request-method
          context.res.setHeader("Access-Control-Max-Age", "1728000"); // valid for 20 days
        }
  ...
  stepProcessPing: function(context, cb) {
    const prefix = this.classRegistry.prefix ? "/" + this.classRegistry.prefix : "";
    if(context.req.path === (prefix + "/ping") || context.req.path === (prefix + "/touch")) {
  ...
  stepCheckEachCallFunction: function(context, cb) {
    if(this.eachCallFn) {
      return this.callEachCallFn(context, cb);
    }
    cb(result.success());
  },
  ...
  stepEndRequest: function(context, cb) {
    ...
    try {
      ...
      var finalResult;
      var success;
      if(context.failedResult) {
        success = false;
        finalResult = context.resultObject || context.invoke_cb_result;
      }
      else if(context.resultObject) {
        // translated means, we got to a point that the result is successful
        finalResult = context.resultObject;
        success = true;
      } else {
        if(cb_result.isType(context.invoke_cb_result)) {
          success = context.invoke_cb_result.success;
          finalResult = context.invoke_cb_result;
        } else {
          success = false;
          finalResult = context.invoke_cb_result;
        }
      ...
      if(config.httpVerbose >= 2) {
        console.log('<<<<', (context.res.statusCode === 304 ? ("" + context.res.statusCode).cyan : ("" + context.res.statusCode).yellow), (cacheHit ||
                                                                                                                                           (success ? finalResult.green : finalResult.red)));
      } else {
  ...
  stepSendResponseHeaders: function(context, cb) {
    if(context.ms) {
      //context.res.setHeader('x-nodetastic-rt', context.ms);
      context.res.setHeader('x-response-time', context.ms);
    }
    var serverTiming = context.serverTiming;
    if(!serverTiming) {
      serverTiming = [];
    }
    serverTiming.push({name: "rt", desc: "Total Response Time", value: context.ms/1000.0});
    var serverTimingHeader = "";
    serverTiming.forEach(function(s) {
      serverTimingHeader += s.name + "=" + s.value + "; \"" + s.desc + "\", ";
    });
  ...
  callEachCallFn: function(context, cb) {
    var self = this;
    self.getParam(context, "$inject", result.cb(cb, function($inject) {
      self.eachCallFn($inject, cb);
    }));
  },
  ...
  stepGetFunctionName: function(context, cb) {
    ...
    if(config.functionNamesCaseInsensitive) {
      context.functionName = context.functionName.toLowerCase();
    }
    context.urlParts = [];
    cb(result.success());
  },
  getFunctionNameWithMethod: function(name, method) {
    return name + "_" + method.toLowerCase();
  },
  stepGetFunctionRef: function(context, cb) {
    ...
    function resolveFromNameFromHandler() {
      ...
      // lookup the function sub parts
      for(var i = 0; fn && i < functionNameParts.length - 1; i++) {
      ...
      // lookup the last function part
      var functionNameWithMethod = self.getFunctionNameWithMethod(functionNameParts[i], context.req.method);
      if(fn[functionNameWithMethod]) {
        fn = fn[functionNameWithMethod];
      } else {
        fn = fn[functionNameParts[i]];
      }
      self.fillFn(context, fn);
      if(!context.fn) {
        return cb(result.errorCode(404, "could not find function"));
      }
      cb(result.success());
  ...
  getReservedValues: function(context, param, circularDependency, cb) {
    ...
    var self = this;
    var isAsync = fn.params.length >= 1 && this.isNameCb(fn.params[fn.params.length - 1].name);
  ...
  fillInParams: function(fn, ignoreFirst, ignoreLast) {
    ...
    var meta = null;
    if(fn.$meta) { // meta is attached to function itself (see function-define)
      meta = fn.$meta;
    } else {
    ...
    fnText = fnText.replace(this.fillInParamsRegex.STRIP_COMMENTS, '');
      ...
      (function(arg) {
        arg.replace(self.fillInParamsRegex.FN_ARG, function(all, underscore, name) {
          ...
          if(name.indexOf("$") === 0) { // starts with $, reserved parameters
            ...
            if(!allowedUntyped && (!type || type.length != 2)) {
  ...
  isNameCb: function(name) {
    return config.cbNames.indexOf(name.toLowerCase()) !== -1;
  },
  fillFn: function(context, fn) {
    if(!fn) {
      return;
    }
    if(typeof(fn) === "function") {
      // simple mode, fn is function
      context.fn = fn;
    } else {
      // advanced mode fn is an object with properties, fn being the function.
      context.fn = fn.fn;
      if(fn.meta) {
        context.meta = fn.meta;
      }
    }
  },
  __end: 0
...
NodeTastic.prototype.registerController = NodeTastic.prototype.registerHandler;
NodeTastic.prototype.controller = NodeTastic.prototype.registerHandler;
NodeTastic.prototype.registerFactory = NodeTastic.prototype.injectReservedValue;
NodeTastic.prototype.factory = NodeTastic.prototype.injectReservedValue;
